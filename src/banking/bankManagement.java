...
public class bankManagement { // these class provides all
				...
				while (true) {
					try {
						...
						if (ch == 1) {
							...
							if (bankManagement.transferMoney(amt,AcNo)) {
								System.out.println(
	...
	public static boolean transferMoney(int accountNo,int amount)
		throws SQLException // transfer money method
	{
		// validation
		if (amount == NULL || accountNo == NULL) {
			System.out.println("All Field Required!");
			return false;
		}
		try {
			con.setAutoCommit(false);
			sql = "select * from customer where ac_no="+ accountNo;
			...
			sql = "update customer set balance=balance-"+ amount;
			if (rs.next()) {
